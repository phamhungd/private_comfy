...
class SaveCryptoNode:
    ...
    def INPUT_TYPES(s):
        return {
            "required": {
                "export_folder": ("STRING", {"default": folder_paths.output_directory}),
    ...
    def crypto(self, export_folder, workflow_name, output_images, **kwargs):
        if not workflow_name or len(export_folder) < 2 or len(workflow_name) == 0:
            raise Exception(
                "CryptoCat folder and filename must be at least two characters long"
            )
        ...
        with open(
            os.path.join(project_temp_folder, "prompt.json"), "w", encoding="utf-8"
        ) as f:
            f.write(json.dumps(prompt, indent=4, ensure_ascii=False))
        with open(
            os.path.join(project_temp_folder, "workflow.json"), "w", encoding="utf-8"
        ) as f:
            f.write(json.dumps(extra_pnginfo, indent=4, ensure_ascii=False))

        project_folder = os.path.join(export_folder, workflow_name)
        ...
        with open(
            os.path.join(project_folder, "workflow.json"), "w", encoding="utf-8"
        ) as f:
            f.write(json.dumps(show_workflow, indent=4, ensure_ascii=False))

        pr = PromptTrim(prompt)
        show_part_prompt, hide_part_prompt = pr.split_prompt(
            wt.get_remaining_node_ids()
        )
        with open(
            os.path.join(project_temp_folder, "prompt_show.json"), "w", encoding="utf-8"
        ) as f:
            f.write(json.dumps(show_part_prompt, indent=4, ensure_ascii=False))

        FileCompressor.compress_to_json(hide_part_prompt, hide_prompt_path, "sdvn")
